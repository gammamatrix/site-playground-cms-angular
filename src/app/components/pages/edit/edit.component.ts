...
export class PagesEditComponent implements OnInit {
  ...
  ngOnInit() {
    this.breakpoint();
    this.fetch();
    console.debug('PagesEditComponent.ngOnInit', {
      id: this.id,
  ...
  fetch() {
    this.service.editInfo(this.id).subscribe(response => {
      ...
      this.isReady = true;
      console.debug('PagesEditComponent.fetch', {
        this: this,
        response: response,
        model: this.model,
      });
  ...
  onSubmit(): void {
    console.debug('PagesEditComponent.onSubmit', {
      this: this,
      editForm: this.editForm.value,
      id: this.id,
      model: this.model,
  ...
  save() {
    if (this.model) {
      ...
      console.debug('PagesEditComponent.save - DISABLED', {
        this: this,
        model: this.model,
      });
    }
  ...
  toggleAccess(formControlName: string, event: MatSlideToggleChange) {
    // console.log('PagesEditComponent.toggle', {
